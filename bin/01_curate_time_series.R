...
deaths_us <- args[2]
confirmed_us <- args[3]
recovered_global <- args[4]
deaths_global <- args[5]
confirmed_global <- args[6]
who_sitreps <- args[7]
wuhan_incidence <- args[8]


# Primarily use time series curated by Johns Hopkins CSSE team
csse_confirmed_original <- read_csv(confirmed_global) %>%
...
csse_confirmed_us <- read_csv(confirmed_us) %>%
...
nejm_wuhan <- read_tsv(wuhan_incidence) %>%
...
early_who_sitreps <- read_tsv(who_sitreps)
...
write_tsv(cumulative_cases, "timeseries_cumulative_cases.tsv")


global_cumulative_cases <- cumulative_cases %>%
  group_by(date) %>%
  summarize(cumulative_cases=sum(cumulative_cases))
write_tsv(global_cumulative_cases, "summary_global_timeseries_cumulative_cases.tsv")
...
country_specific_cumulative_cases <- cumulative_cases %>%
  ...
  lapply(function (x) {
    ...
    output_df <- group_by(x, date) %>%
      summarize(cumulative_cases=sum(cumulative_cases))
    write_tsv(output_df, paste0("summary_", country_name, "_timeseries_cumulative_cases.tsv"))
    output_df
  })
...
province_specific_cumulative_cases <- cumulative_cases %>%
  ...
  lapply(function (x) {
    ...
    output_df <- group_by(x, date) %>%
      summarize(cumulative_cases=sum(cumulative_cases))
    write_tsv(output_df, paste0("summary_", province_name, "_timeseries_cumulative_cases.tsv"))
    output_df
  })
...
new_cases <- cumulative_cases %>%
  ...
  ungroup()
write_tsv(new_cases, "timeseries_new_cases.tsv")


global_new_cases <- new_cases %>%
  group_by(date) %>%
  summarize(new_cases=sum(new_cases))
write_tsv(global_new_cases, "summary_global_timeseries_new_cases.tsv")
...
country_specific_new_cases <- new_cases %>%
  ...
  lapply(function (x) {
    ...
    output_df <- group_by(x, date) %>%
      summarize(new_cases=sum(new_cases))
    write_tsv(output_df, paste0("summary_", country_name, "_timeseries_new_cases.tsv"))
    output_df
  })
...
province_specific_new_cases <- new_cases %>%
  ...
  lapply(function (x) {
    ...
    output_df <- group_by(x, date) %>%
      summarize(new_cases=sum(new_cases))
    write_tsv(output_df, paste0("summary_", province_name, "_timeseries_new_cases.tsv"))
    output_df
  })
